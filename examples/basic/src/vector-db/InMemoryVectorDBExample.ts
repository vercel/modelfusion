...
(async () => {
  ...
  const textEmbeddings = await embedText({ model, texts });
  for (const [i, embedding] of textEmbeddings.entries()) {
  ...
  const queryVector = (
    await embedText({ model, texts: ["rainbow and water droplets"] })
  )[0];

  const results = await vectorDB.retrieve({
    queryVector,
    maxResults: 3,
